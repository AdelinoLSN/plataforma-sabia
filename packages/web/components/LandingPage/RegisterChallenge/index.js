...
const registerChallenge = () => {
	...
	const handleSubmit = async ({ title, description, keywords }, form) => {
		...
		const { data, success } = await createChallenge(
			title,
			description,
		...
		if (success) {
			toast.success('Desafio tecnológico enviado com sucesso!');
		} else {
			toast.error(
				data?.message
					? data?.message
					: 'Ocorreu um erro ao enviar seu desafio tecnológico, tente novamente.',
			);
		}
			...
			<S.Container>
				...
				>
					<S.StyledTitle align="left" noPadding noMargin>
						Conte-nos seu desafio tecnológico
					</S.StyledTitle>
					<InputField
						name="title"
						type="text"
						label="Título do desafio tecnológico"
						validation={{ required: true }}
						variant="gray"
					/>
					<TextField
						name="description"
						label="Detalhe seu desafio"
						validation={{ required: true }}
					...
					<SelectField
						name="keywords"
						placeholder="Adicione palavras-chave ao seu desafio"
						...
						validation={{ required: true }}
						wrapperCss={S.inputWrapperCss}
						instanceId="select-new-challenge"
					/>
